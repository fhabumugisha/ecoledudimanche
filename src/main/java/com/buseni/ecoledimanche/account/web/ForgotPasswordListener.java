...
public class ForgotPasswordListener implements ApplicationListener<ResetPasswordEvent> {
    ...
    public void onApplicationEvent(ResetPasswordEvent event) {
        this.sendResetPasswordMail(event.getAppUrl(),  event.getToken(), event.getUserAccount(), event.getLocale());
    }

      
    public void sendResetPasswordMail(String contextPath, String token, UserAccount account, final Locale locale) {
		String url = contextPath + "/reset-password?id="+ account.getId() + "&token=" + token;
		...
		ctx.setVariable("name", account.getFirstName());
